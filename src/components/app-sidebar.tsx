...
const ChatInput = ({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Attach files: Cmd/Ctrl + Shift + U (avoids View Source conflicts)
      if (actualMetaKey && e.shiftKey && !e.altKey && e.key.toLowerCase() === 'u') {
        e.preventDefault()
        console.log('[AppSidebar] File attach shortcut triggered (Cmd+Shift+U) at', new Date().toISOString())
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (actualMetaKey && !e.shiftKey && !e.altKey && e.key.toLowerCase() === 'k') {
        e.preventDefault()
        console.log('[AppSidebar] New Chat shortcut triggered (Cmd+K) at', new Date().toISOString())
        handleNewChat()
      }
      ...
      // Close sidebar: Cmd/Ctrl + J (commonly used for toggles, safe across all platforms)
      else if (actualMetaKey && !e.shiftKey && !e.altKey && e.key.toLowerCase() === 'j') {
        e.preventDefault()
        console.log('[AppSidebar] Close sidebar shortcut triggered (Cmd+J) at', new Date().toISOString())
        toggleSidebar()
      }
    }
